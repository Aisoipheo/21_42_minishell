...
/*   Updated: 2022/03/10 14:51:27 by rdrizzle         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_export_print(t_info *info, int fds[2])
{
	t_ll_elem	*ptr;
...
int	ft_export(t_llist *args, t_info *info, int fds[2])
{
	...
	ptr = args->head;
	if (args->size == 1)
		return (ft_export_print(info, fds));
	ptr = ptr->next;
	while (ptr)
	{
